...
import {
  User, Calendar, LogOut, CheckCircle, AlertCircle,
  Menu, Shield, Database, Play, FileText, ChevronDown, Mail,
} from 'lucide-react';
...
// ── Normaliza una materia de la BD al formato del frontend ────────────────────
// La BD usa:  id_materia, id_semestre, docente_id, tipo_aula
// El frontend usa: id, semestre, docenteId, tipoAula
function normalizarMateria(m) {
  return {
    ...m,
    // ID
    id:              m.id        ?? m.id_materia,
    // Semestre como número
    semestre:        Number(m.semestre ?? m.id_semestre ?? 0),
    // Docente
    docenteId:       m.docenteId ?? m.docente_id ?? null,
    // Tipo de aula
    tipoAula:        m.tipoAula  ?? m.tipo_aula  ?? 'Aula',
    // Horas desglosadas (con fallback a 0)
...
export default function App() {
  ...
  // Cerrar dropdown de perfil al clic fuera
  useEffect(() => {
    const handler = (e) => {
  ...
  // Cargar datos de la BD cuando el usuario inicia sesión
  useEffect(() => {
    if (!usuario) return;

    // Usuarios (solo DDE los necesita)
    fetch(`${API}/usuarios`)
      .then(r => r.json())
      .then(data => setUsuarios(Array.isArray(data) ? data : []))
      .catch(console.error);

    // Docentes, materias y aulas en paralelo
    Promise.all([
      ...
      .then(([docs, mats, auls]) => {
        ...
        // Normalizar materias al formato del frontend
        setMaterias(Array.isArray(mats) ? mats.map(normalizarMateria) : []);
        setAulas(Array.isArray(auls) ? auls : []);
      })
      .catch(console.error);

    // Última versión del horario (si existe)
  ...
  // ── Login ──────────────────────────────────────────────────────────────────
  const handleLogin = async ({ usuario: user, password }) => {
    try {
  ...
  const onHorarioGenerado = async (horario, horas) => {
    ...
    // Persistir en BD como nueva versión
    try {
      ...
      console.error('[App] Error guardando versión:', err);
    }
  ...
  // ── Horario editado manualmente desde MOD-4 ────────────────────────────────
  ...
  // ── Setter de materias que normaliza automáticamente ──────────────────────
  // Lo usamos en MOD-2 para que cualquier materia que llegue de la BD
  // quede en el formato correcto antes de llegar al AG.
  const setMateriasNorm = (updater) => {
    setMaterias(prev => {
      const next = typeof updater === 'function' ? updater(prev) : updater;
      return Array.isArray(next) ? next.map(normalizarMateria) : prev;
    });
  };
  ...
  const TABS = [
    { id: 'mod1', label: 'Administración del Sistema', icon: <Shield size={16} />,   mod: 'mod1' },
  ...
  return (
    <div style={{ display:'flex', width:'100%', height:'100%', minHeight:'100vh', background:'#f1f5f9', fontFamily:"'Georgia', serif", position:'fixed', inset:0 }}>
      ...
      <main style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        {/* Header */}
        <header style={{ background:'white', padding:'0 28px', height:64, display:'flex', alignItems:'center', justifyContent:'space-between', borderBottom:`3px solid ${C.gold}`, flexShrink:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <h2 style={{ margin:0, fontSize:15, fontWeight:'bold', color:C.navy }}>
              {TABS.find(t => t.id === activeTab)?.label || 'Administración del Sistema'}
            </h2>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            ...
            <div ref={perfilRef} style={{ position:'relative', marginRight:8 }}>
              <button
                ...
                style={{ display:'flex', alignItems:'center', gap:10, background: perfilOpen ? '#f1f5f9' : 'none', border:'1px solid', borderColor: perfilOpen ? '#e2e8f0' : 'transparent', borderRadius:12, padding:'8px 14px', cursor:'pointer', transition:'all 0.15s' }}
              >
                <div style={{ width:38, height:38, borderRadius:'50%', background:C.navy, color:C.gold, display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, fontWeight:'bold', flexShrink:0, border:`2px solid ${C.gold}` }}>
                  {inicial}
                </div>
                ...
                <ChevronDown size={13} color="#94a3b8" style={{ transition:'transform 0.2s', transform: perfilOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} />
              </button>

              {perfilOpen && (
                <div style={{ position:'absolute', top:'110%', right:0, width:260, background:'white', borderRadius:14, border:'1px solid #e2e8f0', boxShadow:'0 8px 32px rgba(0,0,0,0.12)', zIndex:1000, overflow:'hidden' }}>
                  <div style={{ background:C.navy, padding:'18px 20px', display:'flex', alignItems:'center', gap:14 }}>
                    <div style={{ width:50, height:50, borderRadius:'50%', background:C.gold, color:C.navy, display:'flex', alignItems:'center', justifyContent:'center', fontSize:20, fontWeight:'bold', flexShrink:0, border:'2px solid rgba(255,255,255,0.2)' }}>
                      {inicial}
                    </div>
                    <div>
                  ...
                  <div style={{ padding:'14px 18px', borderBottom:'1px solid #f1f5f9' }}>
                    {usuario.email && (
                      <div style={{ display:'flex', alignItems:'center', gap:10, fontSize:12, color:'#64748b', marginBottom:8 }}>
                        <Mail size={14} color="#94a3b8" /><span>{usuario.email}</span>
                      </div>
                    )}
                    <div style={{ display:'flex', alignItems:'center', gap:10, fontSize:12, color:'#64748b' }}>
                      <Shield size={14} color="#94a3b8" /><span>Rol: {usuario.rol}</span>
                    </div>
                  </div>
                  <div style={{ padding:'10px' }}>
                    <button onClick={handleLogout} style={{ width:'100%', display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderRadius:10, border:'none', background:'#fef2f2', color:'#dc2626', cursor:'pointer', fontSize:13, fontWeight:'bold' }}>
        ...
        <div style={{ flex:1, overflow:'auto', padding:20 }} className="fade-in">
          {activeTab === 'mod1' && (
            <Mod1AdminView
              ...
              docentes={docentes} materias={materias} aulas={aulas} grupos={grupos}
              horarioData={horarioData} estadoHorario={estadoHorario}
              historial={historial} addNotif={addNotif} onNavigate={setActiveTab}
            />
          )}
          {activeTab === 'mod2' && (
            <Mod2GestionAcadView
              docentes={docentes}   setDocentes={setDocentes}
              materias={materias}   setMaterias={setMateriasNorm}
          ...
          {activeTab === 'mod5' && (
            <Mod5ValidacionView
              ...
              estado={estadoHorario} onAprobar={onAprobar}
              onVerHorario={() => setActiveTab('mod4')}
              historial={historial} addNotif={addNotif}
            />
          )}
          {activeTab === 'mod6' && (
            <Mod6ReportesView
              horario={horarioData} docentes={docentes} materias={materias}
              aulas={aulas} grupos={grupos} horasDoc={horasDocData}
              estadoHorario={estadoHorario} addNotif={addNotif} usuario={usuario}
            />
          )}
        </div>
      </main>
