/**
 * =============================================================================
 * MÓDULO 5 - VALIDACIÓN Y CALIDAD INSTITUCIONAL
 * Archivo: Mod5ValidacionView.jsx
 *
 * IMPORTANTE PARA INTEGRACIÓN CON BASE DE DATOS:
 * Este componente recibe la prop `tipoUsuario` desde el sistema de autenticación.
 * Los valores esperados son:
 *   - 'ADMIN_DDE'    → Vista del Administrador DDE
 *   - 'JEFE_CARRERA' → Vista del Jefe de Carrera
 *   - 'DOCENTE'      → Vista del Docente de Carrera
 *
 * Las observaciones provienen de dos fuentes:
 *   - `observacionesJefe`   → Array de observaciones que el Jefe envió desde Gestión de Horarios
 *   - `observacionesDocente` → Array de observaciones que el Docente envió desde Gestión de Horarios
 *
 * El Jefe y el Docente solo ven SUS PROPIAS observaciones (historial propio).
 * El DDE ve la lista CONJUNTA de ambos (jefe + docente), sin poder agregar.
 *
 * Estructura esperada de cada observación:
 * {
 *   id: number,
 *   autor: string,
 *   texto: string,
 *   fecha: string,
 *   tipo: 'JEFE_OBS' | 'DOCENTE_OBS'
 * }
 * =============================================================================
 */
...
import {
  CheckCircle, AlertTriangle, Shield, Check, AlertCircle,
  Clock, Archive, User, Bell, FileText,
  BarChart2, Layers, XCircle, Activity, RefreshCw,
...
export function Mod5ValidacionView({
  ...
  // ─── PROP DE INTEGRACIÓN ───────────────────────────────────────────────────
  // tipoUsuario: string — valor inyectado por el sistema de auth.
  // Valores: 'ADMIN_DDE' | 'JEFE_CARRERA' | 'DOCENTE'
  // Si no llega (fallback de seguridad), se usa 'ADMIN_DDE'.
  tipoUsuario,

  // observacionesJefe: array — observaciones que el Jefe envió desde Gestión de Horarios
  // observacionesDocente: array — observaciones que el Docente envió desde Gestión de Horarios
  observacionesJefe = [],
  observacionesDocente = [],
}) {

  // ─── DETECCIÓN DE USUARIO ──────────────────────────────────────────────────
  // tipoUsuario llega desde el sistema de autenticación (prop obligatoria).
  const rolActivo = tipoUsuario || 'ADMIN_DDE';

  // ─── ESTADOS GENERALES ────────────────────────────────────────────────────
  const [submoduloActivo, setSubmoduloActivo] = useState(
    rolActivo === 'DOCENTE' ? 'observaciones' : 'auditoria'
  );
  ...
  // ─── OBSERVACIONES ────────────────────────────────────────────────────────
  // Las observaciones que llegan desde el módulo de Gestión de Horarios
  // se combinan con las observaciones locales (si las hay).
  // JEFE_CARRERA: solo ve sus propias (tipo JEFE_OBS)
  // DOCENTE: solo ve las suyas propias (tipo DOCENTE_OBS)
  // ADMIN_DDE: ve la lista conjunta (JEFE_OBS + DOCENTE_OBS), sin formulario
  const [obsLocalJefe, setObsLocalJefe] = useState(observacionesJefe);
  const [obsLocalDocente, setObsLocalDocente] = useState(observacionesDocente);

  // ─── ESTADO: ¿El Jefe ha enviado alguna observación? ─────────────────────
  // Este flag habilita el botón "Horario con Observaciones" para el Jefe.
  // Se activa cuando el Jefe tiene al menos una observación registrada.
  const jefeHaEnviadoObservaciones = obsLocalJefe.length > 0;
  ...
  const registrarAuditoriaInterna = useCallback((accion, detalle = '') => {
    const nuevoLog = {
      ...
      accion,
      detalle,
      usuario: rolActivo === 'ADMIN_DDE' ? 'Administrador DDE'
             : rolActivo === 'JEFE_CARRERA' ? 'Jefe de Carrera'
             : 'Docente',
  ...
  // ─── COMPONENTE AUXILIAR: FILA DE CHECKLIST ───────────────────────────────
  ...
  // ─── COMPONENTE AUXILIAR: PÍLDORA DE OBSERVACIÓN ─────────────────────────
  const ObsPill = ({ obs, estiloTipo }) => (
    <div className={`institutional-comment-pill ${estiloTipo}`}>
      <div className="comment-meta-row">
        <strong>{obs.autor}</strong>
        <span>{obs.fecha}</span>
      </div>
      <p style={{ margin: 0 }}>{obs.texto}</p>
    </div>
  );
  ...
  return (
    <div className="sagh-validation-container">
      ...
      {/* ====================================================================
          ...
          - ADMIN_DDE:    NO muestra ningún botón de acción (eliminado según spec)
          - JEFE_CARRERA: Muestra "Aprobar Horario" y "Horario con Observaciones"
                          El segundo botón se habilita solo si el Jefe tiene obs.
          - DOCENTE:      Solo visualiza el estado, sin acciones
          ==================================================================== */}
      <div className={`institutional-workflow-banner state-${estadoGlobal.toLowerCase()}`}>
        ...
        <div className="banner-right-side">
          ...
          {/* ----------------------------------------------------------------
              ...
              Botones: "Aprobar Horario" y "Horario con Observaciones"
              "Horario con Observaciones" se habilita solo si el Jefe envió obs.
              ---------------------------------------------------------------- */}
          {rolActivo === 'JEFE_CARRERA' && estadoGlobal === 'EN_REVISION' && (
            <div className="workflow-button-group">
              {/* Botón 1 — Horario con Observaciones (habilitado solo si hay obs del jefe) */}
              <button
                className="workflow-trigger-btn btn-danger"
                disabled={!jefeHaEnviadoObservaciones}
                title={!jefeHaEnviadoObservaciones ? 'Debe registrar observaciones en el módulo de Gestión de Horarios para habilitar esta opción.' : ''}
                onClick={() => registrarTransicionFlujo('BORRADOR', 'Jefatura devuelve horario con observaciones', 'JEFE_CARRERA')}
              >
                Horario con Observaciones
              </button>
              {/* Botón 2 — Aprobar Horario */}
              <button
                className="workflow-trigger-btn btn-success"
                disabled={!sinConflictosCriticos}
                onClick={() => registrarTransicionFlujo('APROBADO', 'Jefatura aprueba el horario formalmente', 'JEFE_CARRERA')}
              >
          ...
          {/* ----------------------------------------------------------------
              VISTA: ADMIN_DDE — Sin botones de acción en el banner
              (El botón "Elevar propuesta a Jefatura" fue eliminado según spec)
              ---------------------------------------------------------------- */}
          {/* rolActivo === 'ADMIN_DDE' → no se renderiza ningún botón aquí */}

          {/* ----------------------------------------------------------------
              VISTA: DOCENTE — Solo visualiza el estado
              ---------------------------------------------------------------- */}
          {/* rolActivo === 'DOCENTE' → solo ve el banner informativo */}

          {estadoGlobal === 'APROBADO' && (
            <span className="immutable-badge">✓ DOCUMENTO BLOQUEADO</span>
          )}
        </div>
      </div>
      ...
      {/* ====================================================================
          ...
          - ADMIN_DDE:    Validación y Calidad | Observaciones | Flujo | Alertas
          - JEFE_CARRERA: Validación y Calidad | Observaciones | Flujo | Alertas
          - DOCENTE:      Solo Observaciones (historial propio)
          ==================================================================== */}
      <div className="institutional-tabs-navbar">

        {/* Tab: Validación y Calidad — visible para DDE y JEFE_CARRERA */}
        {rolActivo !== 'DOCENTE' && (
          <button
            className={`tab-item-link ${submoduloActivo === 'auditoria' ? 'active' : ''}`}
            onClick={() => setSubmoduloActivo('auditoria')}
          >
            Validación y Calidad
          </button>
        )}

        {/* Tab: Historial de Observaciones — visible para todos los usuarios */}
        <button
          className={`tab-item-link ${submoduloActivo === 'observaciones' ? 'active' : ''}`}
          onClick={() => setSubmoduloActivo('observaciones')}
        >
          {/* Etiqueta contextual según rol */}
          {rolActivo === 'DOCENTE' && 'Mis Observaciones'}
          {rolActivo === 'JEFE_CARRERA' && 'Mis Observaciones'}
          {rolActivo === 'ADMIN_DDE' && 'Observaciones Recibidas'}
        </button>

        {/* Tab: Flujo de Aprobación — visible para DDE y JEFE_CARRERA */}
        {rolActivo !== 'DOCENTE' && (
          <button
            className={`tab-item-link ${submoduloActivo === 'flujo' ? 'active' : ''}`}
            onClick={() => setSubmoduloActivo('flujo')}
          >
            Flujo de Aprobación
          </button>
        )}

        {/* Tab: Historial de Alertas — visible para DDE y JEFE_CARRERA */}
        {rolActivo !== 'DOCENTE' && (
          <button
            className={`tab-item-link ${submoduloActivo === 'notificaciones' ? 'active' : ''}`}
            onClick={() => setSubmoduloActivo('notificaciones')}
          >
      ...
      {/* ====================================================================
          CONTENIDO: VALIDACIÓN Y CALIDAD (con sub-pestañas)
          Visible: ADMIN_DDE y JEFE_CARRERA
          - JEFE_CARRERA en estado BORRADOR ve un panel de espera
          ==================================================================== */}
      {submoduloActivo === 'auditoria' && rolActivo !== 'DOCENTE' && (
        <div className="submodule-viewport-card fade-in">

          {/* JEFE_CARRERA: Panel bloqueado mientras el DDE no eleve el horario */}
          {rolActivo === 'JEFE_CARRERA' && estadoGlobal === 'BORRADOR' ? (
            <div className="pending-state-lockout">
              <Shield size={44} className="lockout-icon" />
              <h4>Panel en Espera de Revisión</h4>
              <p>El Administrador DDE se encuentra configurando la matriz. El panel analítico se activará cuando el horario esté disponible para revisión.</p>
            </div>
          ) : (
            <>
              {/* Sub-menú interno: 4 sub-pestañas analíticas */}
      ...
      {/* ====================================================================
          CONTENIDO: HISTORIAL DE OBSERVACIONES
          ─────────────────────────────────────────────────────────────────────
          VISTA: DOCENTE
            → Ve solo las observaciones que él mismo envió (tipo DOCENTE_OBS)
              desde el módulo de Gestión de Horarios.
            → No puede agregar observaciones aquí (se agregan en Gestión de Horarios).

          VISTA: JEFE DE CARRERA
            → Ve solo las observaciones que él mismo envió (tipo JEFE_OBS)
              desde el módulo de Gestión de Horarios.
            → No puede agregar observaciones aquí (se agregan en Gestión de Horarios).

          VISTA: ADMIN_DDE
            → Ve la lista CONJUNTA: observaciones del Jefe + observaciones del Docente.
            → NO puede agregar observaciones (solo lectura).
          ==================================================================== */}
      {submoduloActivo === 'observaciones' && (
        <div className="submodule-viewport-card fade-in">

          {/* ----------------------------------------------------------------
              VISTA: DOCENTE — Historial de sus propias observaciones
              ---------------------------------------------------------------- */}
          {rolActivo === 'DOCENTE' && (
            <div className="observations-workspace-grid">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <MessageSquare size={16} color={C.navy} />
                <h3 className="audit-section-header color-navy" style={{ margin: 0 }}>
                  Mis Observaciones Enviadas
                </h3>
              </div>
              <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>
                Aquí puedes revisar el historial de observaciones que has enviado desde el módulo de Gestión de Horarios.
              </p>
              {obsLocalDocente.length === 0 ? (
                <div className="obs-empty-notice">
                  <ClipboardList size={32} color="#94a3b8" style={{ marginBottom: 8 }} />
                  <p style={{ color: '#64748b', fontSize: 13 }}>No has enviado observaciones aún.</p>
                  <p style={{ color: '#94a3b8', fontSize: 11 }}>Las observaciones se registran desde el módulo de Gestión de Horarios.</p>
                </div>
              ) : (
                <div className="obs-bucket" style={{ gap: 10 }}>
                  {obsLocalDocente.map(obs => (
                    <ObsPill key={obs.id} obs={obs} estiloTipo="teacher-sent" />
                  ))}
                </div>
              )}
            </div>
          ...
          {/* ----------------------------------------------------------------
              VISTA: JEFE DE CARRERA — Historial de sus propias observaciones
              ---------------------------------------------------------------- */}
          {rolActivo === 'JEFE_CARRERA' && (
            <div className="observations-workspace-grid">
              ...
              <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>
                Historial de observaciones que has registrado en el módulo de Gestión de Horarios.
                {!jefeHaEnviadoObservaciones && (
                  <span style={{ color: '#d97706', display: 'block', marginTop: 4 }}>
                    ⚠ Para habilitar el botón "Horario con Observaciones", debes registrar al menos una observación en Gestión de Horarios.
                  </span>
                )}
              </p>
              {obsLocalJefe.length === 0 ? (
                <div className="obs-empty-notice">
                  <ClipboardList size={32} color="#94a3b8" style={{ marginBottom: 8 }} />
                  <p style={{ color: '#64748b', fontSize: 13 }}>No has enviado observaciones aún.</p>
                  <p style={{ color: '#94a3b8', fontSize: 11 }}>Las observaciones se registran desde el módulo de Gestión de Horarios.</p>
                ...
                <div className="obs-bucket" style={{ gap: 10 }}>
                  {obsLocalJefe.map(obs => (
                    <ObsPill key={obs.id} obs={obs} estiloTipo="internally" />
                  ))}
                </div>
              )}
            </div>
          ...
          {/* ----------------------------------------------------------------
              VISTA: ADMIN_DDE — Lista conjunta (Jefe + Docente), solo lectura
              ---------------------------------------------------------------- */}
          {rolActivo === 'ADMIN_DDE' && (
            <div className="observations-workspace-grid">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <h3 className="audit-section-header color-navy" style={{ margin: 0 }}>
                  Observaciones Recibidas — Vista Consolidada
                </h3>
              </div>
              <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 16px' }}>
                Panel de solo lectura. Se muestran todas las observaciones registradas por el Jefe de Carrera y los Docentes en el módulo de Gestión de Horarios.
              </p>

              <div className="observations-split-display">

                {/* Columna izquierda: Observaciones del Jefe de Carrera */}
                <div className="obs-bucket">
                  <h4 className="bucket-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <Shield size={12} /> Observaciones del Jefe de Carrera ({obsLocalJefe.length})
                  </h4>
                  {obsLocalJefe.length === 0 ? (
                    <p className="empty-disclaimer-text">El Jefe de Carrera no ha registrado observaciones.</p>
                  ) : (
                    obsLocalJefe.map(obs => (
                      <ObsPill key={obs.id} obs={obs} estiloTipo="internally" />
                    ))
                ...
                {/* Columna derecha: Observaciones de los Docentes */}
                <div className="obs-bucket">
                  <h4 className="bucket-title" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <User size={12} /> Observaciones de Docentes ({obsLocalDocente.length})
                  </h4>
                  {obsLocalDocente.length === 0 ? (
                    <p className="empty-disclaimer-text">No se registran observaciones de docentes.</p>
                  ) : (
                    obsLocalDocente.map(obs => (
                      <ObsPill key={obs.id} obs={obs} estiloTipo="teacher-sent" />
                    ))
      ...
      {/* ====================================================================
          CONTENIDO: FLUJO DE APROBACIÓN (TRAZABILIDAD)
          Visible: ADMIN_DDE y JEFE_CARRERA
          ==================================================================== */}
      ...
      {/* ====================================================================
          CONTENIDO: HISTORIAL DE ALERTAS EMITIDAS
          Visible: ADMIN_DDE y JEFE_CARRERA
          ==================================================================== */}
      {submoduloActivo === 'notificaciones' && rolActivo !== 'DOCENTE' && (
        <div className="submodule-viewport-card fade-in">
